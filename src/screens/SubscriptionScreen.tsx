...
const SubscriptionScreen = ({navigation, route}:any) => {
    ...
    .then( async (validationReponse)=>{ 
      ...
      if (purchaseType.length==0){
        ...
        if(purchaseType[0] === "NoWatermarks"){
          ...
          setTimeout(() => {
            setLoading(false)  
            returnScreen ? navigation.navigate(`${returnScreen}`, {refetch: true}) : navigation.navigate("CustomScreen", {refetch: true}) 
            //1 // navigation.canGoBack() ? navigation.goBack() : navigation.pop()
            //2 // navigation.canGoBack() ? navigation.pop() :
                // returnScreen ? navigation.push(returnScreen) :
                // navigation.push('CustomScreen')
          }, 2000);
        }
  ...
  const getAppleAccessToken: any = usePostAppleAccessToken({
    onSuccess(res) { 
      storeAppleAccessToken({ access_token: res.access_token, galleryRefresh: true, individualRefresh: true })
      // setLoading(false)  
      // setTimeout(() => {
      //   returnScreen ? navigation.navigate(`${returnScreen}`, {refetch: true}) : navigation.navigate("CustomScreen", {refetch: true}) 
      // }, 1000);
            ...
            <View style={{flexDirection:'row', justifyContent:'space-between', alignItems:'center',  paddingHorizontal:20 }} >
              <TouchableOpacity
                ...
                onPress={()=>{
                  ...
                  returnScreen ? navigation.navigate(`${returnScreen}`) : navigation.navigate("CustomScreen") 

                  // navigation.canGoBack() ? navigation.navigate('IndividualGiphScreen') : navigation.pop()
                  // navigation.canGoBack() ? navigation.goBack() : navigation.pop()
                  // navigation.canGoBack() ? navigation.pop() :
                  // returnScreen ? navigation.push(returnScreen) :
                  // navigation.push('CustomScreen')
