...
const SubscriptionScreen = ({navigation, route}: any) => {
  ...
  const Services = [
    {
      ...
      SVG: <GifsMemes width={40} height={40} style={{marginRight: 10}} />,
    },
    {
      Label: 'No ads!',
      SVG: <NoAds width={40} height={40} style={{marginRight: 10}} />,
    },
  ];
  ...
  const restorePurchases = async () => {
    ...
    await getReceiptIOS({forceRefresh: true})
      .then(async (latestAvailableReceipt: any) => {
        if (latestAvailableReceipt) validateReceipt(latestAvailableReceipt);
      })
      .catch((error: any) => {
        setLoading(false);
      ...
      .then(async (purchaseResponse: any) => {
        await finishTransaction({purchase: purchaseResponse})
          .then(AckResult => {
            ...
            if (AckResult && receipt) validateReceipt(receipt);
          })
  ...
  const validateReceipt = async (receipt: string) => {
    await validateReceiptIos({
      ...
      .then(async validationReponse => {
        ...
        let trialPeriod: boolean | undefined;
        let not_expired: boolean | undefined;
        console.log('action: ', action);
        // Find Subscription
        if (action === 'subscribe') {
          ...
          if (not_expired || trialPeriod) {
            ...
            !purchaseType.includes(subscriptionObject.product_id)
              ? purchaseType.push(subscriptionObject.product_id)
              : null;
          } else {
            setLoading(false);
            SubscriptionAlert(
              'Subscription Expired',
              'Your monthly subscription has expired. Would you like to re-subcribe',
            );
          }
        } // Find Purchases
        else if (action === 'purchase') {
          // Premium Receipt
          getAppleAccessToken.mutate({
            receipt: validationReponse.latest_receipt,
          });

          const purchaseObject = renewal_history?.find(
            (item: {product_id: string}) => item?.product_id === 'NoWatermarks',
          );
        ...
        if (purchaseType.length == 0) {
          ...
          SubscriptionAlert(
            'Payments Alert',
            'Your have no active payments. Would you like to subcribe',
          );
          setLoading(false);
          ...
          if (purchaseType[0] === 'NoWatermarks') {
            setVerifyPayments({
              one_time: true,
              subcription: paymentStatus?.subcription,
              is_trial_period: paymentStatus?.trialPeriod,
            });
            setTimeout(() => {
              setLoading(false);
              navigation.navigate(`${returnScreen}`, {refetch: true});
            }, 2000);
            ...
            setTimeout(() => {
              ...
              returnScreen
                ? navigation.navigate(`${returnScreen}`, {refetch: true})
                : navigation.navigate('CustomScreen', {refetch: true});
            }, 2000);
          }
            ...
            <View style={{alignItems: 'center', marginTop: 30}}>
              ...
              <View style={{alignItems: 'center'}}>
                ...
                <Text
                  ...
                  3 day free trial. Then {subscription[0]?.localizedPrice}{' '}
                  monthly
                </Text>
              </View>
            </View>
