...
const SubscriptionScreen = ({navigation, route}: any) => {
  ...
  const Services = [
    {
      Label: 'All gifs and memes!',
      SVG: (
        <GifsMemes
          width={40 * ratio}
          height={40 * ratio}
          style={{marginRight: 10 * ratio}}
        />
      ),
    },
    {
  ...
  const [checkingSubscriptions, setCheckingSubscriptions] =
  ...
  const validateReceipt = async (receipt: string) => {
    await validateReceiptIos({
      ...
      .then(validationReponse => {
        ...
        if (
          renewal_history[0]?.product_id === 'NoWatermarks' &&
          renewal_history[1]?.product_id === 'MonthlySubscription'
        )
          setVerifyPayments({one_time: true, subcription: true});
        else if (
          renewal_history[0]?.product_id === 'MonthlySubscription' &&
          renewal_history[1]?.product_id === 'NoWatermarks'
        )
          setVerifyPayments({one_time: true, subcription: true});
    ...
    <Fragment>
      <SafeAreaView
        ...
        <View style={{flex: 1, backgroundColor: '#FF439E'}}>
          ...
          <ScrollView
            ...
            <View
              ...
              <TouchableOpacity
                onPress={() => {
                  navigation.canGoBack()
                    ? navigation.pop()
                    : returnScreen
                    ? navigation.push(returnScreen)
                    : navigation.push('CustomScreen');
                }}>
                <BackButton
                  width={RFValue(25 * ratio)}
                  height={RFValue(25 * ratio)}
                />
              </TouchableOpacity>
              ...
              <View style={{flexDirection: 'row', alignItems: 'center'}}>
                <TouchableOpacity
                  ...
                  <Information
                    width={RFValue(22 * ratio)}
                    height={RFValue(22 * ratio)}
                  />
                </TouchableOpacity>
                <TouchableOpacity
                  onPress={() => {
                  ...
                  <Text
                    style={{
                      color: '#ffffff',
                      fontSize: RFValue(14 * ratio),
                      fontWeight: '400',
                      marginLeft: RFValue(10 * ratio),
            ...
            <View style={{alignItems: 'center', marginTop: 30 * ratio}}>
              <AppLogo
                width={RFValue(250 * ratio)}
                height={RFValue(130 * ratio)}
              />
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      style={{
                        flexDirection: 'row',
                        alignItems: 'center',
                        marginVertical: RFValue(10 * ratio),
                      }}>
                      {data.SVG}
                      <Text
                        style={{
                          color: 'white',
                          fontFamily: 'Lucita-Regular',
                          fontSize: RFValue(18 * ratio),
                          paddingVertical: RFValue(10 * ratio),
                        }}>
                        {data.Label}
                      </Text>
                    </TouchableOpacity>
              ...
              {(loading || checkingSubscriptions) && (
                <ActivityIndicator
                  size={'small'}
                  color={'blue'}
                  style={{marginTop: 10 * ratio}}
                />
              )}
              <Subcribe
                width={RFValue(230 * ratio)}
                height={RFValue(30 * ratio)}
                style={{marginTop: RFValue(20 * ratio)}}
              />
              <ArrowDown
                width={RFValue(30 * ratio)}
                height={RFValue(30 * ratio)}
                style={{alignSelf: 'center', marginTop: -2 * ratio}}
              />
              <TouchableOpacity
                onPress={() => {
                ...
                style={{
                  ...
                  padding: RFValue(15 * ratio),
                  borderRadius: RFValue(15),
                  marginTop: RFValue(10 * ratio),
                }}
                disabled={subscriptions.length < 1 || loading ? true : false}>
                <Text
                  style={{
                    color: '#ffffff',
                    fontSize: RFValue(20 * ratio),
                    fontFamily: 'Lucita-Regular',
                  }}>
                  Try Free & Subscribe
                </Text>
              ...
              <Text
                style={{
                  color: 'white',
                  fontSize: RFValue(10 * ratio),
                  paddingTop: RFValue(5 * ratio),
                  fontFamily: 'Lucita-Regular',
                  alignSelf: 'center',
                }}>
                3 day free trial. Then {subscriptions[0]?.localizedPrice}{' '}
                monthly
              </Text>
            </View>
          </ScrollView>
          ...
          <View style={{alignItems: 'center', backgroundColor: '#3386FF'}}>
            <TouchableOpacity
              ...
              style={{
                ...
                padding: RFValue(12 * ratio),
                borderRadius: RFValue(15),
                marginTop: RFValue(20 * ratio),
              }}
              disabled={products.length < 1 || loading ? true : false}>
              <Text
                style={{
                  color: '#622FAE',
                  fontSize: RFValue(12 * ratio),
                  fontFamily: 'Lucita-Regular',
                }}>
                No Watermarks{' '}
              </Text>
        ...
        <AppModal isVisible={isVisibleModal} setModalVisible={setVisibleModal}>
          <AppModal.Container>
            <AppLogo
              width={RFValue(150 * ratio)}
              height={RFValue(60 * ratio)}
              style={{alignSelf: 'center', marginVertical: 20 * ratio}}
            />
            <TouchableOpacity
              onPress={() => setVisibleModal(false)}
              style={{
                position: 'absolute',
                right: RFValue(10 * ratio),
                top: RFValue(10 * ratio),
              }}>
              <Cross
                width={RFValue(22 * ratio)}
                height={RFValue(22 * ratio)}
            ...
            <AppModal.Body ratio={ratio}>
              <View style={{margin: 10 * ratio}}>
                <Text
                  ...
                  Subscription renews monthly or annually. Payment will be
                  charged to iTunes account at confirmation of purchase.
                  Subscription automaticlaly renews unless the automatic-renew
                  is turned off at least 24 hours before the end of the current
                  period. Account will be charged for renewal within 24 hours
                  prior to the end of the current period, and identify the cost
                  of the renewal subscription may be managed by the user and
                  auto-renewal may be turned off by going to the user’s Account
                  Settings after purchase. Any unused portion of a free trial
                  period, if offered, will be forfeited when the user purchases
                  a subscription to that application. By signing up for
